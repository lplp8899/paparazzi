...
int find_contour(char *img, int width, int height)
{
  ...
  for (int row = 0; row < height; row++) {
    for (int col = 0; col < width; col++) {
      // Extract pixel color from image
      Vec3b &M_in =  M.at<cv::Vec3b>(row, col);
      Vec3b &M_out =  thresh_image.at<cv::Vec3b>(row, col);
      //int img_ij_1 = int( M.at<cv::Vec3b>(row, col).val[0] );
      //int img_ij = int(M.at<cv::Vec3b>(row, col)[1] );

      if (col % 2 == 0) 
      { 
         if ( ( M_in[1] > y1 ) && (M_in[1]<y2) && ( M_in[0] > u1 ) && (M_in[0]<u2) )
         {  M_out = 255;
            //M_out[1] = 255;
            //M_out[2] = 255; 
         }
      ...
      if (col % 2 == 1) 
      { 
         if ( ( M_in[1] > y1 ) && (M_in[1]<y2) && ( M_in[0] > v1 ) && (M_in[0]<v2) )
         {  M_out = 255;
            //M_out[1] = 255;
            //M_out[2] = 255; 
         }
...
/*
void pos_err_est_call(float dx, float dy, float dz)
{
  dx = d_x;
  dy = d_y;
  dz = d_z;
